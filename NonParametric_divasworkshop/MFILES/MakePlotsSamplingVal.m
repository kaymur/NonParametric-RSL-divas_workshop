function truth = MakePlotsSamplingVal(dataset,f2s_shifted,upp,low,testloc,labl,difftimestep,xlim0,meanSL,testt,coral_params,RSLs,Seed,modno)
%function makeplots_colored_samp(dataset,f2s_shifted,sd2s,V2s,testloc,labl,doplots,difftimestep,xlim0,maxdistfrom,meanSL,testt,coral_params)
...
if ~isstruct(testloc)
    if testt==0
        testlocs.X=1950-dataset.meantime;
...
%     True_RSL1=interp3(GIALat,GIALon,GIAtime,GIA0,Flat,Flong,firstyears,'linear');
%     True_RSL2=interp3(GIALat,GIALon,GIAtime,GIA0,Flat,Flong,lastyears,'linear');
        truth=interp3(RSLs.Lat,RSLs.Lon,RSLs.Time,RSLs.Y0,testlocs.X(:,1),testlocs.X(:,2),testlocs.X(:,3),'linear');
        %GIALon,GIAtime,GIA0,testloc.sites,testlocs.X,'linear');
    end
...
%    testX(:,1:3)=[mean(dataset.lat)*ones(length(testt),1) mean(dataset.long)*ones(length(testt),1) 1950-testt];
...
if length(RSLs)>0
    truth=interp3(RSLs.Lat,RSLs.Lon,RSLs.Time,RSLs.Y0,testloc.X(:,1),mod(testloc.X(:,2),360),testloc.X(:,3),'linear');
else
    truth=[];
end
...
%         hold on;
[tx,ti]=sort(testX(subA,3));
        plot(1950-testX(subA(ti),3),f2s(subA(ti))+offsetA,'Color',[.5, .5, .5]);
            hold on;
        plot(1950-testX(subA(ti),3),upper(subA(ti))+offsetA,'--','Color',[.5, .5, .5]);
 		plot(1950-testX(subA(ti),3),lower(subA(ti))+offsetA,'--','Color',[.5, .5, .5]);
if isempty(truth)
else
        plot(1950-testX(subA(ti),3),truth(subA(ti)),'r','LineWidth',1);
end    %plot(testX(subA(ti),3),f2s(subA)+sd2s(subA)+offsetA,'--','Color',[.5, .5, .5]);
        % 		plot(testX(subA(ti),3),f2s(subA)-sd2s(subA)+offsetA,'--','Color',[.5, .5, .5]);
% 		plot(testX(subA(ti),3),f2s(subA)+2*sd2s(subA)+offsetA,':','Color',[.5, .5, .5]);
% 		plot(testX(subA(ti),3),f2s(subA)-2*sd2s(subA)+offsetA,':','Color',[.5, .5, .5]);
    hold on;
    if ~exist('ylim0', 'var')
        ylim([-50e3 25e3]);
    ...
    title(testnames{i});
%    title=[labl num2str(testsites(i,1))];
%    pdfwrite(['Post_95_no_data' '_' testnames{1}]);  
    if ~exist('ylim0', 'var')
        ylim([-80e3 25e3]);    
    else
        ylim(ylim0)
    end
    pdfwrite([labl 'Post_95_no_data_bounds_' num2str(Seed)]);  
...
if modno == 3
    for nn = 1:length(subB)
        if limiting(subB(nn))==0&&cspecies(subB(nn))==0
            rectangle('Position',[meantime(subB(nn))-2*dt(subB(nn)),Y(subB(nn))-2*dY(subB(nn)),4*dt(subB(nn)),4*dY(subB(nn))],'Edgecolor',[0,0,0]);  % peat index points basal, black
                %min(time1(subB(nn)),time2(subB(nn))),Y(subB(nn))-2*dY(subB(nn)),meantime(subB(nn))-2*dt(subB(nn)),meantime(subB(nn))-2*dt(subB(nn)),4*dY(subB(nn))],'Edgecolor',[0,0,0]);  % peat index points basal, black
                %dYears(time2(subB(nn)),time1(subB(nn))),
        elseif limiting(subB(nn))==0&&cspecies(subB(nn))>=10
            rectangle('Position',[meantime(subB(nn))-2*dt(subB(nn)),Y(subB(nn))-2*dY(subB(nn)),4*dt(subB(nn)),4*dY(subB(nn))],'Edgecolor',[0,0,0]);  % peat index points basal, black
                %min(time1(subB(nn)),time2(subB(nn))),
%         elseif limiting(subB(nn))==0&&cspecies(subB(nn))==9
%                 rectangle('Position',[min(time1(subB(nn)),time2(subB(nn))),Y(subB(nn))+coral_params(cspecies(subB(nn)),1)*1000-2000*coral_params(cspecies(subB(nn)),2),...
%                 dYears(time2(subB(nn)),time1(subB(nn))),4000*coral_params(cspecies(subB(nn)),2)],'Edgecolor',colors(cspecies(subB(nn)),:));  % peat index points basal, turquoise
%             else
%                 plot([1 1]*meantime(subB(nn)),[Y(subB(nn))-2*dY(subB(nn)) Y(subB(nn))+4*means(cspecies(subB(nn)))*1000+2*dY(subB(nn))],'Color',colors(cspecies(subB(nn)),:));
%                 plot([time1(subB(nn)) time2(subB(nn))],[1 1]*Y(subB(nn)),'Color',colors(cspecies(subB(nn)),:));
        elseif cspecies(subB(nn))==9&&limiting(subB(nn))==0
                plot([1 1]*meantime(subB(nn)),[Y(subB(nn))-2*dY(subB(nn)) Y(subB(nn))+2*means(cspecies(subB(nn)))*1000+2*dY(subB(nn))],'Color',colors(cspecies(subB(nn)),:));
                plot([meantime(subB(nn))-4*dt(subB(nn)) meantime(subB(nn))+2*dt(subB(nn))],[1 1]*Y(subB(nn)),'Color',colors(cspecies(subB(nn)),:));
                    %time1(subB(nn)) time2(subB(nn))],[1 1]*Y(subB(nn)),'Color',colors(cspecies(subB(nn)),:));
        elseif limiting(subB(nn))==0&&cspecies(subB(nn))==1
            rectangle('Position',[meantime(subB(nn))-2*dt(subB(nn)),Y(subB(nn))+coral_params(cspecies(subB(nn)),1)*1000-2000*coral_params(cspecies(subB(nn)),2),...
                4*dt(subB(nn)),4000*coral_params(cspecies(subB(nn)),2)],'Edgecolor',colors(cspecies(subB(nn)),:));  % peat index points basal, turquoise
        elseif limiting(subB(nn))==2
            plot([1 1]*meantime(subB(nn)),[Y(subB(nn))-2*dY(subB(nn)) Y(subB(nn))+means(cspecies(subB(nn)))*1000+2*dY(subB(nn))],'Color',colors(cspecies(subB(nn)),:));
            plot([meantime(subB(nn))-4*dt(subB(nn)) meantime(subB(nn))]+2*dt(subB(nn)),[1 1]*Y(subB(nn)),'Color',colors(cspecies(subB(nn)),:));
                %time1(subB(nn)) time2(subB(nn))]
        elseif limiting(subB(nn))==3 % uniform
            rectangle('Position',[meantime(subB(nn))+2*dt(subB(nn)),Y(subB(nn))-2*dY(subB(nn)),4*dt(subB(nn)),4*dY(subB(nn))],'Edgecolor',[.8,.8,.8]);  % uniform, grey
            hold on
            %[min(time1(subB(nn)),time2(subB(nn))),
        end
    end
...
%         col{i}
            ...
            hold on
%       plot(testX(subA(ti),3),f2s(subA)+offsetA,'Color',[.5, .5, .5]);
% 		plot(testX(subA(ti),3),f2s(subA)+sd2s(subA)+offsetA,'--','Color',[.5, .5, .5]);
% 		plot(testX(subA(ti),3),f2s(subA)-sd2s(subA)+offsetA,'--','Color',[.5, .5, .5]);
% 		plot(testX(subA(ti),3),f2s(subA)+2*sd2s(subA)+offsetA,':','Color',[.5, .5, .5]);
% 		plot(testX(subA(ti),3),f2s(subA)-2*sd2s(subA)+offsetA,':','Color',[.5, .5, .5]);
...
% 
		...
		if length(wxlim)==0
    		wxlim = xlim0;
%     		wxlim(1) = max(floor(min(testX(subA(ti),3))/100)*100-1000,xlim0(1));
            %   if length(subB)>0
...
% sl_rate{i}=[difftimes(subA2),df2s(subA2),dsd2s(subA2)];
    pdfwrite([labl 'Post_95_data_' num2str(Seed)]);  
