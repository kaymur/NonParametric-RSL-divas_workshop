function [f2s,sd2s,V2s,testlocs,logp,passderivs,invcv]=RegressHoloceneDataSets_sampled(dataset,testsitedef,modelspec,thetTGG,trainsub,noiseMasks,testt,refyear,collinear,passderivs,invcv,y_samp,t_samp,sub0)
...
if ~isempty(t_samp)
    dt=zeros(length(meantime));
    meantime(sub0)=t_samp;
else
...
% dY=0*dataset.dY;
% % use sampled Y instead of original Y

% compaction correction
if length(y_samp) > 0 %exist('y_samp', 'var')
%    dY=15*ones(length(Y));
    Y(sub0)=y_samp;
    Ycv(sub0,sub0)=sparse(diag(dY(sub0).^2));
end

testsites=testsitedef.sites;
testnames=testsitedef.names;
testnames2=testsitedef.names2;
if isfield(testsitedef,'te')
...
if ~isempty(passderivs)
    ...
    if max(dt)>0
        ...
        try
         ...
         % f2s,sd2s,V2s,testlocs,logp,passderivs,invcv  
          logp=-1e12;
        end
    else
